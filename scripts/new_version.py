...
apps = ["digital_agartala", "digital_agartala_admin"]
reltype = ""
hostingsite = ""
rootdir = ""
...
def main():
    ...
    # Prompt which app to update
    print("Select app to update version:")
    for index, app in enumerate(apps, start=1):
        print(f"{index}. {app}")
    both_choice = str(len(apps) + 1)
    print(f"{both_choice}. both")
    appid = input(f"Enter app choice (1-{both_choice} or both): ").strip().lower()
    if appid.isdigit() and 1 <= int(appid) <= len(apps):
        selected_apps = [apps[int(appid) - 1]]
    elif appid in [both_choice, "both"]:
        selected_apps = apps
    else:
    ...
    try:
        ...
        for app in selected_apps:
            ...
            set_value_in_file(
                f"{app}/pubspec.yaml", "version", f"{newversion}+{old_build_number + 1}"
            )
